...
def booking_page():
    ...
    st.markdown(
        """
        Use the calendar below to book a 30-minute call.
        We’ll review your assessment results and outline
        clear, practical next steps.
        """
    )

    # Optional: log booking intent via n8n later
    # payload = {"session_id": st.session_state.session_id}

    components.iframe(
        src=f"https://mautic.goxmit.com/form/14?session_id={st.session_state.session_token}",
        height=700,
        scrolling=True
    )
    st.markdown("---")
