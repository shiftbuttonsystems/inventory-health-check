...
base_url = st.sidebar.text_input("Baserow URL", "http://baserow.goxmit.com")
table_id = st.sidebar.text_input("Table ID", "759")
token = st.secrets['BASEROW_TOKEN']

# Test functions
def test_connection():
    """Test if we can connect to Baserow"""
    url = f"{base_url}/api/database/rows/table/759/?user_field_names=true"
